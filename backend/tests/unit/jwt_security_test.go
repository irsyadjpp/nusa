...
func TestJWTSignatureForgery(t *testing.T) {
	...
	service := jwtService.NewService(secret, accessExpiry, refreshExpiry, issuer)
...
func TestJWTClaimsTampering(t *testing.T) {
	...
	wrongService := jwt.NewService(wrongSecret, accessExpiry, refreshExpiry, issuer)
...
func TestJWTTokenLeeway(t *testing.T) {
	...
	shortExpiryService := jwtService.NewService(secret, 100*time.Millisecond, refreshExpiry, issuer)
